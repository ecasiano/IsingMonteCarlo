...
# COMPUTE ERROR BARS #
    ...
    plt.plot(T,chi,'-o',label=r'$L=%d,T_{\rm{max}}=%.2f$'%(L,T[np.argmax(chi)]),alpha=alpha,zorder=z_order)
#     plt.plot(T,chi,'-',alpha=1/(0.1*L))
    plt.ylabel(r'$\langle \chi / N \rangle$')
    plt.xlabel("T");
    plt.axvline(Tc_exact,zorder=-100,color='gray')
    plt.legend(frameon=False)
    
plt.savefig("../Figures/d_susceptibility.svg",dpi=400)